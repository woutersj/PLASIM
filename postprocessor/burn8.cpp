...
void NetOpen(char *NetFileName)
{
   ...
   NetFile = new NcFile(NetFileName,NcFile::replace,NcFile::classic64);
   ...
   LonDim = NetFile->addDim("lon" , londim);
   LatDim = NetFile->addDim("lat" , Lats  );
   LevDim = NetFile->addDim("lev" , OutLevs  );
   TimDim = NetFile->addDim("time"        );

   LonVar = NetFile->addVar("lon" , ncDouble, LonDim);
   LatVar = NetFile->addVar("lat" , ncDouble, LatDim);
   LevVar = NetFile->addVar("lev" , ncDouble, LevDim);
   TimVar = NetFile->addVar("time", ncDouble, TimDim);
   ...
   {
      LevVar.putAtt("axis"         ,"Z"                          );
      LevVar.putAtt("long_name"    ,"sigma at layer midpoints"   );
      LevVar.putAtt("standard_name","atmosphere_sigma_coordinate");
      LevVar.putAtt("positive"     ,"down"                       );
      LevVar.putAtt("units"        ,"level"                      );
   }
   else // pressure level
   {
      LevVar.putAtt("axis"         ,"Z"            );
      LevVar.putAtt("long_name"    ,"pressure"     );
      LevVar.putAtt("standard_name","pressure"     );
      LevVar.putAtt("units"        ,"hPa"          );
   }

   TimVar.putAtt("calendar"     ,cale           );
   TimVar.putAtt("units"        ,t_unit         );
   TimVar.putAtt("base_date", ncInt, 4 ,BaseDate       );

   
   NetFile->putAtt("title"      , title);
   NetFile->putAtt("history"    , hist );
   NetFile->putAtt("Conventions", conv );

   for (jvar = 0 ; jvar < nattr ; ++jvar)
   {
      NetFile->putAtt(AttrNam[jvar],AttrVal[jvar]);
   }
   
   LonVar.putVar(Outlon->Lam);
   LatVar.putVar(Outlat->Phi);
   LevVar.putVar(Outlev);
...
void NetVarDefine(void)
{
   ...
   if (All[jvar].selected)
   {
      ...
      {
         vector<NcDim> dims;
         dims.push_back(TimDim);
         dims.push_back(LatDim);
         dims.push_back(LonDim);
         if (RealSize == 8)
            All[jvar].NetVar = NetFile->addVar(All[jvar].Id,ncDouble,dims);
         else
            All[jvar].NetVar = NetFile->addVar(All[jvar].Id,ncFloat,dims);
      }
      else
      {
         vector<NcDim> dims;
         dims.push_back(TimDim);
         dims.push_back(LevDim);
         dims.push_back(LatDim);
         dims.push_back(LonDim);
         if (RealSize == 8)
            All[jvar].NetVar = NetFile->addVar(All[jvar].Id,ncDouble,dims);
      ...
      All[jvar].NetVar.putAtt("long_name"    ,All[jvar].Na);
      All[jvar].NetVar.putAtt("standard_name",All[jvar].Na);
      All[jvar].NetVar.putAtt("units"        ,All[jvar].Un);
      All[jvar].NetVar.putAtt("code"         , ncInt,   jvar    );
      if (GaussianOutput)
      All[jvar].NetVar.putAtt("grid_type"    ,"gaussian"  );
   }
}
